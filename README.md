...
## Training Method
* colab pro의 TPU 이외의 버전에서 학습된 모델을 불러올시 문제가 발생함(GPU 및 CPU로 이미지를 생성할시 이미지 깨짐 현상 발생)<br>
1. 학습하기 위해 마련한 모든 데이터를 한번에 pipeline에 넣을 경우 colab에서 할당하는 ram이 버티지를 못함<br>
...
## 문제점
1. 모델을 학습할 때는 모델의 크기가 너무 커서 64x64로 이미지를 잘라서 넣을 수 밖에 없는 문제가 발생함  
2. TPU 이외의 환경에서 이미지 생성시 이미지가 깨지는 문제가 발생  
* 이 문제를 해결하기 위한 방법을 찾는 것은 또 다른 과제  
